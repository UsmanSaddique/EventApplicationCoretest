...
namespace EventApplicationCore.Concrete
{
    public class VenueConcrete : IVenue
    {
        ...
        public IEnumerable<Venue> ShowAllByUser(int ?id)
        {
            var listdata = (from a in _context.Venue
                            join b in _context.Registration on a.Area equals b.Area
                            where b.ID == id
                            select a
                          ).ToList();
            return listdata;
        }
        public IEnumerable<Venue> ShowAllVenue(int ?id)
        {
            var value = _context.Registration.Where(x => x.ID == id).Select(c => c.Area).FirstOrDefault();
            var listdata = _context.Venue.Where(x => x.Area != value).ToList();
            return listdata;
        }
